...
function refreshShadow() {
    ...
    console.log('[EVENT] refreshShadow(): Update shadow:', JSON.stringify(toUpdate));
...
thingShadow.on('status', function(thingName, stat, clientToken, stateObject) {
    ...
    if (clientToken === clientTokenUpdate) clientTokenUpdate = null;
});
...
thingShadow.on('delta', function(thingName, stateObject) {
    ...
    console.log('[EVENT] thingShadow.on(delta): Updated thingState to:', JSON.stringify(thingState));
